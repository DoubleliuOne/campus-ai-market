...
```powershell
Set-Location -LiteralPath 'D:\CampusAI Market\docs-site'
npm install
npm run dev
```

推送 `main` 后，GitHub Actions 会构建并部署到 GitHub Pages。首次部署前需在仓库 `Settings -> Pages` 将 Source 设置为 `GitHub Actions`。
